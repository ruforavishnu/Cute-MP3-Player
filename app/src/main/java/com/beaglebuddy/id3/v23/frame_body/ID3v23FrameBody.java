 ...
 * </table>
 * <p class="beaglebuddy">
 * This is the base class for all ID3v2.3 frame bodies.  Although <a href="http://www.id3.org">ID3v2.3</a> defines 74 different types of frames, many groups of frame bodies have the
 * same fields, and thus are implemented by a single class.  For example, there are 38 <i>text information</i> frames, and thus the corresponding frame bodies are implemented in one
 * {@link ID3v23FrameBodyTextInformation} class.  Similarly, there are 8 <i>url link</i> frames, but all are implemented using the {@link ID3v23FrameBodyURLLink} class.
 * </p>
 * <p class="beaglebuddy">
 ...
 * given frame body.  When a frame body is read in from an .mp3 file (more accurately, from the ID3v2.3 tag in the .mp3 file), this binary buffer is parsed and the values are stored in
...
public class ID3v23FrameBody extends ID3v23FrameBodyUtility
{
    ...
    * @param frameType  the type of ID3v2.3 frame that is to be created.
    */
    ...
    * @param frameType      the type of ID3v2.3 frame to create.
    ...
    */
   public ID3v23FrameBody(InputStream inputStream, FrameType frameType, int frameBodySize) throws IOException
   {
...
*/
    ...
    * @throws IOException     if there is an error while reading in the frame body.
    */
   public ID3v23FrameBody(InputStream inputStream, String invalidFrameId, int frameBodySize) throws IOException
   {
          ...
          frameBodySize > 0 && frameBodySize < 200000)
      {
         ...
         if (inputStream.read(buffer) != buffer.length)
            throw new IOException("Error reading the body of an invalid ID3v2.3 frame with id " + invalidFrameId + ".");
      }
      else
      {
         throw new IOException("The mp3 file contains a corrupt ID3v2.3 tag with an invalid frame (" + invalidFrameId + ") which is not readable.");
      }
   }

   /**
    * gets the frame body's ID3v2.3 type.
    * @return the frame body's ID3v2.3 type.
    */
   ...
   public void parse() throws IllegalArgumentException
   {
      ...
      //       however, when the ID3v23Frame class reads in a frame header with an invalid frame id, then it creates an instance of this class
    ...
    * <p>
    * this method should be abstract.  However, the ID3v23Frame constructor needs to be able to instanitate instances of a frame body
   ...
   {
      if (dirty)
         throw new IllegalStateException("The ID3v2.3 frame " + frameType.getId() + " has been modified and requires setBuffer() to be called before it can be saved.");
