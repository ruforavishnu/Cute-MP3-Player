 ...
 * </table>
 * <p class="beaglebuddy">
 * This class provides methods for reading and writing ID3v2.4 frames.  As the .mp3 file format shows below, an {@link com.beaglebuddy.id3.v24.ID3v24Tag ID3v2.4 tag} is
 ...
 * track number, the lyrics of the song, the band who recorded the song, etc. are stored in the frames.  The <a href="http://id3.org/id3v2.4.0-structure">ID3v2.4</a> Specification
 * defines 83 different {@link com.beaglebuddy.id3.enums.v24.FrameType types of frames}.  A frame consists of a {@link ID3v24FrameHeader header} and a
 * {@link com.beaglebuddy.id3.v24.frame_body.ID3v24FrameBody body}. The frame header contains some information about the frame body (such as its size, type, etc.) and as
 * such does not vary much between the different frame types.  This is in contrast to the {@link com.beaglebuddy.id3.v24.frame_body.ID3v24FrameBody body}, which contains
 ...
 *       <tr><td class="beaglebuddy">2. </td><td class="beaglebuddy">an invalid frame id is encountered                                            </td><td class="beaglebuddy">The <a href="http://id3.org/id3v2.4.0">ID3v2.4 Specification</a> defines 83 frame types, each with its own unique 4 character
 *                                                                                                                                                                              {@link com.beaglebuddy.id3.enums.v24.FrameType frame id}.  If the Beaglebuddy MP3 library encounters a frame with an invalid frame id, then it tries
 ...
 *                                                                                                                                                                              the rest of the .mp3 file can be read in, the bad frame discarded, and the .mp3 file saved so that it contains only valid ID3v2.4 frames.           </td></tr>
 *       <tr><td class="beaglebuddy">3. </td><td class="beaglebuddy">a valid frame id was read, but it contained an invalid value in the frame body</td><td class="beaglebuddy">This is the most common type of error that you will encounter.  In this case, we have one of the valid 83 frame types, but the frame contains some
 ...
 *                                                                                                                                                                              {@link com.beaglebuddy.id3.enums.v24.Encoding character encoding} (must be 0 - 3), an invalid {@link com.beaglebuddy.id3.enums.TimeStampFormat time
...
{
   // data members
   private ID3v24FrameHeader header;
   private ID3v24FrameBody   body;
   private String            invalidMessage;  // indicates whether the frame read in is a valid ID3v2.4 frame
    ...
    * @param frameType   the ID3v2.4 type of frame to create.
    */
   public ID3v24Frame(FrameType frameType)
   {
      // most frame body classes have a default constructor.
      // however, ID3v24FrameBodyTextInformation and ID3v24FrameBodyURLLink do not, since they are used to instantiate multiple frame body type and therefore require a frame type
      ...
      try
      {
         ...
         header         = new ID3v24FrameHeader(frameType);
         frameBodyClass = frameType.getFrameBodyClass();
         if (frameBodyClass.getName().endsWith("ID3v24FrameBodyTextInformation") || frameBodyClass.getName().endsWith("ID3v24FrameBodyURLLink"))
         {
            constructor = frameBodyClass.getConstructor(new Class<?>[] {FrameType.class});
            body        = (ID3v24FrameBody)constructor.newInstance(frameType);
         }
         else
         {
            constructor = frameBodyClass.getConstructor(new Class<?>[] {});
            body        = (ID3v24FrameBody)constructor.newInstance();
         }
      }
   ...
   /**
    * constructor. called when creating a new frame.
    * @param frameType    the ID3v2.4 id of the type of frame to create.
    * @param frameBody    the ID3v2.4 frame body corresponding to the frame type being created.
    * @throws IllegalArgumentException   if the class of the frame body does not correspond that specified by the <i>frameType</i>.
    *                                    For example, if you have a frameType of FrameType.ATTACHED_PICTURE and a frame body of type ID3v24FrameBodyTextInformation instead of ID3v24FrameBodyAttachedPicture.
    */
   public ID3v24Frame(FrameType frameType, ID3v24FrameBody frameBody) throws IllegalArgumentException
   {
      ...
      if (!frameBodyClass.equals(frameBody.getClass()))
         throw new IllegalArgumentException("Invalid frame body type, " + frameBody.getClass().getName() + " specified.  ID3v2.4 frame id requires a frame body of type " + frameBodyClass.getName() + ".");
    ...
    * @param  inputStream                input stream pointing to the next frame in the ID3v2.4 tag.
    ...
    *                                    since all the bytes for the frame were read, this frame will simply be marked as being invalid, and processing of the remainder of the ID3 v2.4 tag will continue.
    */
   public ID3v24Frame(ID3v24FrameHeader header, InputStream inputStream) throws IOException, IllegalArgumentException
   {
      this.header = header;
      ...
      try
      {
         ...
         body           = (ID3v24FrameBody)(constructor.getParameterTypes().length == 2 ? constructor.newInstance(inputStream,                        header.getFrameBodySize())
      ...
      catch (NullPointerException ex)
      {
         ...
         {
            // try and read in the bytes of the body so that we can continue parsing the remaining frames in the ID3v2.4 tag
            body           = new ID3v24FrameBody(inputStream, header.getInvalidFrameId(), header.getFrameBodySize());
            invalidMessage = "Invalid ID3v2.4 frame id " + header.getInvalidFrameId();
         }
         else
         {
    ...
    * @return the ID3v2.4 frame header;
    */
   public ID3v24FrameHeader getHeader()
   {
      return header;
   }
    ...
    */
   public void setHeader(ID3v24FrameHeader header)
   {
      this.header = header;
   }
    ...
    * @return the ID3v2.4 frame body;
    */
   public ID3v24FrameBody getBody()
   {
      return body;
   }

   /**
    * sets the ID3v2.4 frame's body to one of the 74 types defined by the <a href="http://www.id3.org">ID32.4</a> Specification.
    * @param body    the frame's body.
    */
   public void setBody(ID3v24FrameBody body)
   {
      this.body = body;
   }
   ...
   /**
    * indicates whether the frame read in is a valid ID3v2.4 frame. This prevents invalid frames that are encountered while parsing an .mp3 file from interrupting the parsing of the rest
    ...
    *  @return whether the frame id read in from the .mp3 file is a valid ID3v2.4 frame id.
    */
    ...
    * @return if the frame is invalid, the reason why the frame is not a valid ID3v2.4 frame is returned.  Otherwise, null is returned.
    */
    ...
    * @return the ID3v2.4 description of the frame.
    */
    ...
    * @param file   file output stream pointing to the starting location of the ID3v2.4 tag within the .mp3 file.
    ...
    * @return a string representation of the ID3v2.4 tag frame.
    */
   public String toString()
   {
      StringBuffer buffer = new StringBuffer();

      buffer.append("ID3v2.4 frame: " + getDescription() + "\n");
